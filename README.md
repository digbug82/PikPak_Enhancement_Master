...
* 新增 **Android Chromium 移动端管理界面适配**，支持在 `360-480px` 竖屏及常见触屏横屏下打开增强管理器。
* 优化 **移动端导航与核心操作**，侧栏改为底部横向导航，工具栏支持触控滚动，文件网格在窄屏下使用双列布局。
* 新增 **移动端更多菜单**，集中提供分析、查重、批量重命名、迁移、云归档等次级操作。
* 修复 **移动端目录列表上传菜单** 被工具栏裁剪导致点击无反馈的问题，并将外部播放器与下载器按钮恢复为直接显示。
* 优化 **移动端弹窗与媒体查看**，适配安全区、全屏视频 / 图片查看及触控卡片操作。
* 优化 **悬浮球触控交互**，使用 Pointer Events 支持轻触打开、拖动定位与位置记忆。
