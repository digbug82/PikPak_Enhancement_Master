...
* 修复图片在极端缩放或长图模式下，无法拖拽至图片最边缘的边界限制计算错误问题。
...
### V1.2.0
* 视频时长嗅探引擎重构。
* 精修部分 UI 组件在不同缩放比例下的视觉表现。
* 修复路径检索时高亮失效的 Bug；修复在设置中保存“极速模式”修改时会导致其他设置项被阻断失效的 Bug。

### V1.1.0
* Aria2支持**保留文件夹路径结构**，新增投递丢包提醒及错误清单一键导出。
* 云下载支持批量提交与智能去重，**自动清洗防屏蔽污染磁链**（支持 Base32 自动解码）。
* 批量重命名预览界面新增**图标与封面展示**，支持文件夹悬浮大图预览。
* 导入功能升级为**“智能合并”**模式，导入备份时不再覆盖本地已有的名单和记录。
* 大幅优化极速模式加载速度，修复账号切换时的 UI 残留 Bug，提升搜图成功率。
