...
* 优化 **下载加速**，新增自定义模板模式、预览与校验和直链重新范围控制范围。
* 优化 **浏览器下载与上传任务冲突处理**，降低触发下载时误触页面离开、刷新拦截或上传中断提示的风险。
* 优化 **PotPlayer链路**，降低部分设备使用 Potplayer 渲染 Pin 失败的风险。
* 优化 **以图搜图**，支持选中带真实封面的文件夹后直接以封面搜图。
* 优化 **云端错误提示**，给出更准确的提示。
* 优化 **分享预览播放稳定性**，增加卡死检测，分享视频预览受限时可更快进入失败兜底提示，避免长时间转圈。
* 优化 **上传任务保护**，暂停状态也纳入活跃上传任务判断，降低上传暂停时误关闭、误刷新导致任务状态异常的风险。
* 优化 **输入长度限制**，对输入框的文本上限边界进一步规范。
* UI与稳定性优化。
