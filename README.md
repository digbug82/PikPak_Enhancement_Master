...
> **移动端已适配**：V5.0.0 起，脚本针对手机及窄屏触控设备重构了核心界面，文件浏览、搜索、批量操作、媒体播放及主要弹窗均提供响应式布局，同时保持桌面端原有布局与操作习惯。
...
* **响应式文件管理**：适配手机横竖屏与安全区域，提供移动端底部导航、紧凑工具栏、两列网格及专用列表布局。
* **触控与批量操作**：支持长按进入选择模式、全选 / 取消全选、批量重命名、排序、文件夹置顶及统一的更多操作菜单。
* **搜索与返回导航**：提供可折叠搜索栏，支持搜索结果高亮；浏览器或系统返回键可依次关闭搜索、菜单、弹窗和媒体预览，并恢复目录历史。
* **媒体手势**：支持上下滑动切换图片或视频、横向拖动视频进度、长按倍速，以及图片双指缩放、单指拖动、双击放大和长图浏览。
...
* **云归档**：支持将选中文件中的可追溯磁链归档到「链接收藏夹 > 云归档」，可删除源文件以节省空间。
...
* **分享解析**：支持解析分享链接，并在保存到网盘前查看分享内容；未保存前不会占用自己的网盘空间。
* **分享文件透视**：支持对分享内容进行递归扫描、筛选和预览，适合在保存前检查分享内部结构。
* **分享解析历史**：可记录已解析的分享链接及提取密码，便于后续快速重新打开。
* **分享管理**：支持设置分享提取次数上限；达到设定上限后可自动取消分享，使链接失效。
* **秒传文件**：上传文件时若服务器已有相同文件记录，可直接完成秒传，减少重复上传等待时间。
* **上传保护**：内置上传期间离开页面提醒，并针对上传中断、残留任务和异常文件进行保护与清理。
* **云下载增强**：支持批量离线链接自动去重；内置磁链智能清洗，可提取 Base32 / Hex 哈希并剔除干扰文本。
* **TXT 磁链提取**：支持预览 TXT 文本，并从文本中识别磁链、HTTP、FTP、ed2k、thunder 等下载链接提交云下载。
* **磁链预览**：当磁链内容存在公开预览信息时，支持显示文件数量、总大小、类型、哈希值及多张预览图。
...
> ⚠️ 分享提取次数上限的自动拦截功能，仅在网页保持开启且设备未休眠时生效。

---

### 媒体播放增强

* **播放引擎增强**：支持 0.5x - 3.0x 倍速、旋转与翻转、强制比例、自动跳过片头片尾、连续播放、循环播放与进度条缩略图预览。
* **默认打开方式**：桌面端可在脚本播放器与 PotPlayer 之间选择，移动端可在脚本播放器与 VLC 之间选择；两端偏好独立保存，并支持默认视频清晰度与外部播放清晰度设置。
* **视频截帧**：支持为视频生成 16 张时间点画面截帧，可进行全选、反选、批量保存、以图搜图及单帧定位播放。
* **音乐播放器**：支持音频文件在线播放、播放列表、顺序播放、随机播放、单曲循环、弹窗模式、封面显示与音量控制。
* **字幕系统**：支持云端同名字幕加载、本地字幕导入、在线字幕搜索；支持字幕时间轴偏移、字幕位置、字号和背景透明度调整。
* **视觉辅助**：支持使用图片或视频当前帧进行以图搜图，可选择 Google Lens、Yandex、SauceNAO、trace.moe 引擎，用于反查封面、演员、番剧或素材来源。
* **图片与视频连续浏览**：可选择将图片和视频加入同一媒体队列连续浏览，并可在播放器内收藏或删除当前文件。
* **媒体模式**：可在设置中启用媒体模式，使仅包含视频或仅包含图片的目录默认按名称 A-Z 排序，提升剧集 / 漫画浏览连续性。
...
* **下载器类型**：支持 Aria2 兼容 RPC、Gopeed API、ABDM API 与 IDM 清单导出；移动端使用 Gopeed 时不会覆盖桌面端下载器偏好。
* **目录结构还原**：推送整个文件夹时可自动恢复云盘中的树状目录结构，避免下载后目录被打平。
* **视频下载加速**：视频下载可优先尝试使用播放链路获取原画直链，提升下载速度。
* **下载过滤**：支持按文件大小、扩展名、名称关键词过滤文件，可用于浏览器下载及向外部下载器分发。
* **下载直链加速域名**：支持下载加速域名，提供拼接原始链接、URL 参数传递和自定义模板三种反向代理改写模式。
...
* **配置管理与清理**：支持导出脚本配置并在其他设备上使用，导入配置合并去重，并支持按配置类型清理数据。
...
* **多账号数据迁移**：支持将选中项目加密一键打包，并登录另一个账号后自动识别并接管，实现数据跨账号转存。
...
4. **启动增强大师**：
   * 普通模式下：登录后点击侧栏浮动的蓝色 **PikPak Logo 悬浮球** 进入。
...
**Q：多层文件夹中，如何通过路径栏快速切换目录层级？**  
**A：** 将鼠标移动到顶部路径栏区域，然后使用鼠标滚轮即可逐级切换路径：

* 鼠标滚轮向下滚动一次，进入下一级目录；
* 鼠标滚轮向上滚动一次，返回上一级目录。

每滚动一次只切换一级，便于在多层文件夹之间快速浏览。

**Q：可以直接通过路径栏移动文件吗？**  
**A：** 可以。选中文件或文件夹后，将其拖拽到路径栏中对应的目标文件夹路径上，松开鼠标即可将项目移动到该目录。

拖拽时请将项目准确放置在目标文件夹名称或路径区域上。若目标路径不可用，或当前页面属于搜索结果及其他虚拟列表，可能无法执行移动操作。
...
**A：** 这类情况通常与 PikPak 官方视频流尚未准备就绪，或浏览器解码状态异常有关。
...
**A：** 不是。这通常是因为 PikPak 云端服务器触发了并发或频率限制，属于官方接口当前繁忙，而不是脚本本身异常。

如果某个文件多次重试后仍然失败，可能说明该资源在云端已经损坏、受限，或当前服务器状态不佳。建议稍后重试，或将文件下载到本地后再处理。
...
* 优化 **移动端文件浏览与批量操作**，统一卡片和列表视图的更多菜单，支持长按选择、全选 / 取消全选、批量重命名、移动端排序及文件夹置顶；切换列表、网格或屏幕方向时可保持滚动位置、选中状态与当前项目。
* 优化 **移动端搜索与返回导航**，新增可折叠搜索栏，回收站支持本地搜索与结果高亮；浏览器或系统返回键会优先关闭当前搜索、菜单、弹窗及媒体预览，并正确恢复虚拟模式和真实目录的导航历史。
* 新增 **PikPak 分享代码智能识别**，在主搜索框输入分享代码或分享链接后，可确认并直接进入分享解析；无法唯一识别时继续执行普通搜索。
* 全面升级 **移动端图片与视频手势**，支持上下滑动切换媒体、横向拖动预览并定位视频进度、单击显隐控件、双击播放 / 暂停、长按 2× / 3× 倍速，以及图片双指缩放、单指拖动、双击放大和长图边界切换。
* 优化 **移动端媒体播放体验**，原生全屏可根据视频方向自动旋转，切换视频时保留封面直至首帧就绪；播放列表、字幕、画质及倍速面板完成短屏适配，并增强图片加载失败、音频误判为视频及移动端音频解码失败时的兜底处理。
* 新增 **图片与视频连续浏览**，支持在同一媒体队列中跨图片和视频连续切换，并可在播放器内收藏、取消收藏或删除当前文件；相关设置支持本地保存与配置云同步。
* 新增 **移动端 VLC 外部播放**，支持将 VLC 设为默认打开方式并应用下载加速地址；未能成功拉起 VLC 时会自动复制播放链接，且移动端 VLC 与桌面端 PotPlayer 配置相互独立。
* 优化 **移动端下载器适配**，移动端使用 Gopeed 时不会覆盖桌面端下载器偏好，保存目录交由 Gopeed 管理，并保持外部播放器与下载器快捷入口可见。
* 优化 **分享解析与配置云同步稳定性**，补充分享文件夹数量、大小展示及扫描中止保护；上传配置前会合并本地与云端分享历史，并检测远端数据变化以避免意外覆盖，上传失败时可恢复原有本地数据。
* 修复 **配置保存与媒体队列问题**，完善“播完暂停 / 列表循环 / 单集循环”播放模式的旧值迁移，修复 IDM EF2 导出类型及文件夹置顶默认值不一致的问题，并改进删除进度校验、列表复用和响应式切换时的状态清理。
* 调整 **站点适配范围**，新增 `mypikpak.net` 与 `pikpak.me`，移除 `app.mypikpak.com` 的脚本匹配。
* UI 与稳定性优化。
...
* 修复 **下载器连接测试与配置提示弹窗**，解决 Aria2 / Motrix、Gopeed、ABDM 等安装指引中的链接或 HTML 标签直接显示为代码的问题，并安全保留受信任的跳转链接。
* 优化 **下载加速设置交互**，未启用“下载时使用加速地址”时，自动隐藏链接传递方式、加速地址、生效范围及模板预览；启用后再显示相关配置。

### V4.6.0

* 新增 **视频截帧**，支持为单个视频生成 16 张画面截帧，并提供全选、反选、批量保存、以图搜图及单帧定位播放功能。
* 优化 **访问验证与登录令牌恢复机制**，修复 V4.5.0 中验证码有效期判断、失效令牌复用及恢复入口不完整导致的频繁鉴权失效，提升自动恢复能力。
* 修复 **播放历史进度读取与续播异常**，完善官方播放进度的读取与恢复。
* UI 与稳定性优化。

### V4.5.0

* 优化 **网格视图流畅性**，改进大量文件和文件夹场景下的渲染、滚动与切换体验。
* 优化 **数据稳定性**，改进目录加载、分页续载、缓存同步、文件操作回写、访问验证恢复及后台扫描调度，降低数据丢失、重复、回退和任务卡住的概率。
* UI 与稳定性优化。
...
* 新增 **默认打开方式** 设置，可在脚本播放器与 PotPlayer 之间选择默认播放方式。
...
* 修复 Aria2 / Motrix 推送 0KB 文件、行高异常、图标显示不一致、英文列宽与路径下拉字体等问题。
