...
* **批次重新命名**：支援正則替換 / 刪除、劇集流水號、文字格式化、FC2 規範命名、前綴去廣告，以及基於 MIME 的副檔名智慧修復。
...
* **分享檔案透視**：支援對分享內容進行遞迴掃描與篩選，便於在儲存前預覽分享內部結構。
...
1. **安裝腳本管理器**：推薦使用 [ScriptCat](https://scriptcat.org/)、[Violentmonkey](https://violentmonkey.github.io/get-it/) 或 [Tampermonkey](https://www.tampermonkey.net/)。
...
當目錄層級較深，前面的上層資料夾路徑被隱藏時，可以將滑鼠移到頂部路徑列區域，然後滾動滑鼠滾輪。路徑列會橫向滾動，把被隱藏的上層路徑顯示出來。

顯示後，直接點擊對應的上層資料夾名稱，即可快速返回上一層，或跳轉到任意上層目錄。

**Q：檔案查重 / 資料夾查重時，勾選出來的檔案會自動保留一個原檔案嗎？還是需要我在清單裡再篩選一次？**  
**A：** 檔案查重和資料夾查重裡的勾選項，本質上都是**實際操作項**。查重模式本身不會直接刪除檔案，只會列出可能重複的項目。您需要確認並勾選後，再點擊刪除，才會真正清理。

在查重結果中：

* 每個分組表頭的勾選框，表示選中這一整組；
* 組內排序只負責依時間、大小、路徑、名稱等規則重新排列顯示，不會自動決定刪除誰、保留誰；
* 只有點擊**一鍵勾選**時，腳本才會依規則自動處理：每組保留 1 個，其餘項目會被勾選。

另外，檔案查重裡的**依資料夾選擇**，是將重複清單中某個資料夾內的檔案勾選；**反向選取**則是將重複清單中除該資料夾以外的檔案勾選。
...
**A：** 腳本裡的「精準比對」是依**檔案內容雜湊**判斷的，不是依檔名判斷。只有當檔案雜湊值和檔案大小都一致時，才會被判定為精準重複。
...
**A：** 不是。這通常是因為 PikPak 雲端伺服器觸發了併發頻率限制，屬於官方介面目前繁忙，而不是腳本本身異常。
...
當檔案是在檔案查重、文件透視、資料夾透視、最近加入、收藏夾等**聚合檢視或虛擬路徑**中被批次刪除時，這些檔案本身可能來自多個不同的原始資料夾。刪除後再從回收站還原，官方還原機制有時不會逐一恢復到原來的父目錄，而可能會把大量檔案集中還原到某一個統一目錄中，導致原來的多層級目錄結構被破壞。

建議在檔案查重、文件透視、資料夾透視、最近加入、收藏夾等聚合結果中批次刪除前，先確認這些檔案是否確實不再需要。尤其是檔案來自多個不同目錄時，盡量避免刪除後再依賴回收站批次還原。
...
**A：** 請先檢查您的網路硬碟剩餘空間是否足夠。PikPak 官方目前對「空間不足」通常採用**靜默攔截**策略，也就是在容量超限時，不一定會跳出明確提示，而是直接在後台中止操作。
...
* **本地優先**：本腳本所有核心能力均透過瀏覽器直接與 PikPak 官方 API 互動，您的帳號 Token、密碼金庫及大多數本地配置數據預設保存在本地瀏覽器環境中。
* **零收集**：腳本**不會主動收集**使用者隱私數據，也**絕不會**將您的檔案資訊或帳號憑證上傳至任何第三方伺服器。
* **第三方介面**：僅在使用「線上字幕搜尋」或「以圖搜圖」等擴充功能時，腳本才會向相關公共服務傳送必要的搜尋關鍵字或圖片特徵參數，不涉及您的個人身分資訊。
...
* 將專案授權協議調整為 **AGPL-3.0-or-later**，移除非商業使用限制，並保留修改版開源要求。
...
* 新增 **分享解析模式**，支援分享連結解析、剪貼簿識別、遞迴透視、檔案儲存、媒體預覽與壓縮檔處理。
* 新增 **下載直連加速網域設定**，支援前綴模式與查詢參數模式，可用於瀏覽器下載與 Aria2 / Motrix 推送連結改寫。
* 新增播放器滑鼠滾輪調節音量，支援音量浮層提示與靜音圖示狀態。
* 優化網頁全螢幕播放器、窄螢幕適配、搜尋高亮、壓縮檔預覽與設定儲存提示。
* 修復 Aria2 / Motrix 推送 0KB 檔案、行高異常、圖示統一、英文欄寬與路徑下拉字型等問題。
...
* 優化網格檢視、影片播放體驗、設定視窗、密碼金庫彈窗、最近加入與播放歷史數據取得。
* 清理除錯輸出、冗餘配置與低風險死碼，降低腳本體積。
...
* UI 與架構升級，以圖搜圖按鈕外置，新增網格檢視。

### V1.3.2

* 本地上傳邏輯加固，增加中國大陸 IP 直連。
* 調整檔案查重的時長相似閾值，優化檢測精度。
...
* 雲端下載支援批次提交與智慧去重，支援自動清洗污染磁鏈與 Base32 自動解碼。
* 批次重新命名預覽介面新增圖示與封面展示，支援資料夾懸浮大圖預覽。
* 匯入功能升級為智慧合併模式，匯入備份時不再覆蓋本地已有名單和紀錄。
