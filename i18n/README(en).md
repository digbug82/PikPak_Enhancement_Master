...
> **Mobile support is here**: Starting with V5.0.0, the core interface has been rebuilt for phones and narrow touch-screen devices. File browsing, search, bulk operations, media playback, and major dialogs now use responsive layouts while preserving the familiar desktop layout and workflow.
...
* **Responsive file management**: adapts to portrait and landscape phone screens and safe areas, with mobile bottom navigation, a compact toolbar, a two-column grid, and a dedicated list layout.
* **Touch and bulk operations**: supports long-press selection, select all / deselect all, batch rename, sorting, folders first, and a unified More actions menu.
* **Search and back navigation**: provides a collapsible search bar with highlighted results. The browser or system Back button closes search, menus, dialogs, and media previews in order, then restores directory history.
* **Media gestures**: supports swiping up or down to switch images or videos, horizontal scrubbing through video, long-press playback speed, plus pinch-to-zoom, single-finger panning, double-tap zoom, and long-image browsing.
...
* **Share Parsing History**: can record parsed share links and extraction passwords for quick reopening later.
* **Share management**: supports setting an access-count limit for shares. When the configured limit is reached, the share can be automatically cancelled to invalidate the link.
...
* **Upload protection**: includes a reminder when leaving during an upload, plus protection / cleanup for interrupted uploads, residual tasks, and abnormal files.
...
* **TXT magnet extraction**: supports previewing TXT text, identifying magnet, HTTP, FTP, ed2k, thunder, and other download links, and submitting them to Cloud Download.
* **Magnet preview**: when public preview information is available, displays file count, total size, type, hash value, and multiple preview images.
...
* **Default opening method**: choose between the script player and PotPlayer on desktop, or between the script player and VLC on mobile. Preferences are stored independently, with settings for default video quality and external playback quality.
* **Video frame capture**: supports generating 16 frame captures from a video, with select all, invert selection, batch saving, image search, and single-frame playback positioning.
...
* **Continuous image and video browsing**: optionally place images and videos in the same media queue for uninterrupted browsing, with controls to favorite or delete the current file from the player.
...
* **Downloader Type**: supports Aria2-compatible RPC, Gopeed API, ABDM API, and IDM list export. Using Gopeed on mobile does not overwrite desktop downloader preferences.
...
* **Direct download acceleration domain**: supports a download acceleration domain, with three reverse-proxy rewrite modes: original-link concatenation, URL parameter forwarding, and custom templates.
...
**Q: How can I quickly switch directory levels using the path bar?**  
**A:** Move the cursor over the top path bar and use the mouse wheel:

* Scroll down once to enter the next directory level;
* Scroll up once to return to the parent directory.

Each scroll switches only one level, making it easier to browse deeply nested folders.

**Q: Can I move files directly through the path bar?**  
**A:** Yes. Select a file or folder, drag it onto the corresponding target folder path in the path bar, and release the mouse to move it there.

Place the item on the target folder name or path area. The move operation may be unavailable when the target path is invalid or the current page is a search result or another virtual list.
...
**A:** This is usually related to PikPak's official video stream not being ready yet or to the browser decoding state, and is not necessarily a script-side judgment error.
...
**A:** No. This usually means the PikPak cloud server has triggered concurrent or frequency limits. It is the official API being busy, not an exception in the script itself.

If a file still fails after multiple retries, the resource may be damaged, restricted, or affected by poor server status in the cloud. Try again later, or download the file locally before handling it.
...
* Completely overhauled the **mobile core interface** with systematic responsive support for narrow screens and touch devices, improving layout consistency and usability across screen sizes and orientations.
* Improved **mobile file browsing and bulk operations** with a unified More menu across card and list views, long-press selection, select all / deselect all, batch rename, mobile sorting, and folders first. Scroll position, selection, and the current item are preserved when switching between list and grid views or rotating the screen.
* Improved **mobile search and back navigation** with a collapsible search bar plus local search and result highlighting in the Recycle Bin. The browser or system Back button now closes the active search, menu, dialog, or media preview first and correctly restores navigation history for virtual modes and real directories.
* Added **smart PikPak share-code recognition**. Entering a share code or share link in the main search box can now open Share Parser after confirmation; ambiguous input continues as a normal search.
* Completely upgraded **mobile image and video gestures** with vertical swipes to switch media, horizontal dragging to preview and seek video, tap to show or hide controls, double-tap to play / pause, long-press for 2x / 3x speed, and image pinch zoom, single-finger panning, double-tap zoom, and boundary-aware long-image navigation.
* Improved the **mobile media playback experience**. Native fullscreen can rotate to match video orientation, and the cover remains visible while switching videos until the first frame is ready. Playlist, subtitle, quality, and playback-speed panels now fit short screens, with stronger fallbacks for image load failures, audio misidentified as video, and mobile audio decoding failures.
* Added **continuous image and video browsing**, allowing images and videos to be traversed in one media queue. The current file can be favorited, unfavorited, or deleted in the player, and the setting supports local persistence and cloud configuration sync.
* Added **external VLC playback on mobile**, including setting VLC as the default opening method and applying the download acceleration URL. If VLC cannot be launched, the playback link is copied automatically. Mobile VLC and desktop PotPlayer configurations are independent.
* Improved **mobile downloader support**. Using Gopeed on mobile no longer overwrites desktop downloader preferences, the save directory is left to Gopeed, and shortcuts for the external player and downloader remain visible.
* Improved **Share Parser and cloud configuration sync reliability** with share-folder count and size display plus scan-abort protection. Local and cloud share history is merged before upload, remote changes are detected to prevent accidental overwrites, and original local data is restored if upload fails.
* Fixed **configuration persistence and media queue issues**, including migration of legacy values for Pause after playback / Playlist loop / Single-item loop, inconsistent IDM EF2 export types and default folders-first values, plus improved deletion progress validation, list reuse, and state cleanup during responsive transitions.
* Updated **site compatibility** by adding `mypikpak.net` and `pikpak.me` and removing the userscript match for `app.mypikpak.com`.
...
* Fixed **downloader connection test and setup dialogs**, preventing links or HTML tags in the Aria2 / Motrix, Gopeed, and ABDM setup instructions from appearing as literal code while safely preserving trusted links.
* Optimized **download acceleration settings**. When “Use an acceleration URL for downloads” is disabled, the link transfer method, acceleration URL, application scope, and template preview are hidden; they are shown after the option is enabled.

### V4.6.0

* Added **video frame capture**, supporting 16 frame captures for a single video, with select all, invert selection, batch saving, image search, and single-frame playback positioning.
* Optimized **access verification and login-token recovery**, fixing frequent authentication failures caused by captcha validity checks, reuse of invalid tokens, and incomplete recovery entry points in V4.5.0, and improving automatic recovery.
* Fixed **watch-history progress loading and resume**, improving the reading and restoration of official playback progress.
* UI and stability optimizations.

### V4.5.0

* Optimized **grid view smoothness**, improving rendering, scrolling, and switching when working with large numbers of files and folders.
* Optimized **data stability**, improving directory loading, pagination resumption, cache synchronization, file-operation write-back, access-verification recovery, and background scan scheduling to reduce data loss, duplication, rollback, and stalled tasks.
* UI and stability optimizations.

### V4.4.0

* Added **My Shares link export**, supporting CSV export of share link lists in “My Shares” by selected items, search results, or the currently loaded list.
* Added **share parsing history export**, supporting CSV export of share links from history records.
* Optimized **My Shares management logic**, supporting batched loading, incremental display, and resuming from the previous loading point after switching modes; after cancelling shares, the list is no longer cleared and reloaded, and only successfully cancelled items are removed.
* Optimized **upload mode stability**, fixing “View in folder” errors after abnormal captcha states, improving file icon display after uploads complete, and disabling file dragging.
* Optimized **archive preview dialog**, using the official icon of the selected archive and fixing abnormal hiding of some text areas.
* UI and stability optimizations.
...
Special thanks to [@yebuwudong](https://github.com/yebuwudong) for submitting a mobile adaptation proposal in [PR #9：add mobile manager layout and touch support](https://github.com/digbug82/PikPak_Enhancement_Master/pull/9). Its exploration of a fullscreen manager, bottom navigation, mobile action menus, touch interactions, and responsive layouts provided valuable reference and inspiration for the V5.0.0 mobile overhaul.
